...
- The headline navigation-quality metric is **solo nav-collection %** — pickups ÷ item-goal
  attempts for a *single* bot alone on the map (combat impossible), which isolates last-leg
  item-collection. A second **deathmatch ITEM%** (5 bots) tracks the same ratio under real combat +
  contention; it runs systematically lower (combat interrupts routes) and is the integration metric.
...
[`STATS.md`](STATS.md); each snapshot pins the nav baseline and seed so rows isolate code changes.
The narrative milestones below track **deathmatch** ITEM completion on q2dm1 (the 5-bot rig, used
throughout ozbot's history; the solo nav-collection headline was added later as the cleaner
nav-quality read — see [`STATS.md`](STATS.md)):
