...
# Dual metric: each map is run once per VARIANT.  A variant = (name, nav_dir, bots).
#   solo (1 bot, combat impossible) -> the nav-QUALITY headline: isolates last-leg
#     item-collection with no combat interruption / respawn-teleport / contention.
#   deathmatch (5 bots) -> the integration/product metric (combat + contention; the
#     only place wins like bot_claim show up).  Combat is off purely by bot_count 1
#     -- no separate game mode.  Both variants share the one pinned nav baseline.
DM_BOTS = 5
SOLO_BOTS = 1
DM_VARIANT = "deathmatch"
SOLO_VARIANT = "solo"
VARIANTS = [(SOLO_VARIANT, PINNED_NAV, SOLO_BOTS), (DM_VARIANT, PINNED_NAV, DM_BOTS)]
...
def run_map(engine, exe, dll_name, mapname, args, bots):
    ...
    sim_args = types.SimpleNamespace(
        repro=False, fastsim=True, bots=bots, skill=args.skill,
...
def _variant(rec_maps, m, variant):
    """Metrics dict for one map's variant, or None.  Shim: a pre-dual-metric flat
    record (metrics keys at the top level, no variant nesting) is the deathmatch rig."""
    d = rec_maps.get(m)
    if not d:
        return None
    if "item_completion" in d or "pickups" in d:      # old flat record == deathmatch
        return d if variant == DM_VARIANT else None
    return d.get(variant)


def _mean_item(rec, variant):
    rm = rec.get("maps", {})
    vals = [d["item_completion"] for m in rm
            for d in [_variant(rm, m, variant)]
...
def _note(rec):
    ...
    return note[:37] + "..." if len(note) > 40 else note
...
def write_stats_md(history):
    ...
    lines.append("")
    lines.append("Each map is run twice per snapshot from a fixed seed + pinned nav baseline "
                 "(`baselines/nav/`), so rows isolate **code** changes:")
    lines.append("")
    lines.append("- **Solo nav-collection %** (headline) — 1 bot, combat impossible. Isolates "
                 "navigation / last-leg item-collection with no combat interruption, "
                 "respawn-teleport, or contention. This is the number to watch for nav / "
                 "locomotion work.")
    lines.append("- **Deathmatch ITEM %** (integration) — 5 bots. The product metric under real "
                 "combat + item contention; the only place combat/contention wins (e.g. "
                 "`bot_claim`) show up. Collection is systematically lower here because combat "
                 "interrupts routes — that's expected, not a nav regression.")
    lines.append("")
    lines.append("The nav baseline is *normalized*: all 8 maps are cold-matured with one "
                 "identical rig via `py tools/benchmark.py --mature`.")
    lines.append("")
    ...
    # --- headline: solo nav-collection ---
    lines.append("### Solo nav-collection (nav quality — 1 bot, no combat)")
    lines.append("")
    lines.append("| Map | Collection % | Pickups | Attempts | Nav nodes |")
    lines.append("|---|---:|---:|---:|---:|")
    for m in maps:
        d = _variant(lm, m, SOLO_VARIANT)
        if not d:
            lines.append(f"| {m} | — | — | — | — |")
            continue
        lines.append(f"| {m} | {fmt_pct(d.get('item_completion'))} | {d.get('pickups','—')} | "
                     f"{d.get('item_attempts','—')} | {d.get('nav_nodes','—')} |")
    lines.append(f"| **mean** | **{fmt_pct(_mean_item(latest, SOLO_VARIANT))}** | | | |")
    lines.append("")
    ...
    lines.append(f"| **mean** | **{fmt_pct(_mean_item(latest, DM_VARIANT))}** | | | | | | |")
    lines.append("")

    # per-item breakdown for the latest snapshot (from the deathmatch run -- richer)
    lines.append("<details><summary>Per-item pickups (latest, deathmatch)</summary>")
    lines.append("")
    for m in maps:
        d = _variant(lm, m, DM_VARIANT)
    ...
    lines.append("## Solo nav-collection over time")
    lines.append("")
    lines.append(header)
    lines.append(sep)
    for rec in history:
        rm = rec.get("maps", {})
        cells = [fmt_pct(d.get("item_completion")) if (d := _variant(rm, m, SOLO_VARIANT))
                 else "—" for m in maps]
        lines.append(f"| {rec.get('date','?')[:10]} | {_note(rec)} | " + " | ".join(cells)
                     + f" | {fmt_pct(_mean_item(rec, SOLO_VARIANT))} |")
    lines.append("")

    # --- integration trend: deathmatch ITEM% over time ---
    lines.append("<details><summary>Deathmatch ITEM % over time</summary>")
    lines.append("")
    lines.append(header)
    lines.append(sep)
    for rec in history:
        rm = rec.get("maps", {})
        cells = [fmt_pct(d.get("item_completion")) if (d := _variant(rm, m, DM_VARIANT))
                 else "—" for m in maps]
        lines.append(f"| {rec.get('date','?')[:10]} | {_note(rec)} | " + " | ".join(cells)
                     + f" | {fmt_pct(_mean_item(rec, DM_VARIANT))} |")
    lines.append("")
    lines.append("</details>")
    lines.append("")

    # frags trend (deathmatch; symmetric self-play -> activity, not skill)
    lines.append("<details><summary>Total frags over time (deathmatch; activity, not skill)</summary>")
    lines.append("")
    lines.append(header.replace(" mean ", " total "))
    lines.append(sep)
    for rec in history:
        rm = rec.get("maps", {})
        vs = [_variant(rm, m, DM_VARIANT) for m in maps]
        cells = [str(d.get("frags", "—")) if d else "—" for d in vs]
        tot = sum(d.get("frags", 0) for d in vs if d)
        lines.append(f"| {rec.get('date','?')[:10]} | {_note(rec)} | " + " | ".join(cells) + f" | {tot} |")
    lines.append("")
    lines.append("</details>")
    lines.append("")
...
def main(argv):
    ...
    ap.add_argument("--bots", type=int, default=DM_BOTS, help="bots for the deathmatch variant")
    ap.add_argument("--solo-bots", type=int, default=SOLO_BOTS,
                    help="bots for the solo (nav-quality) variant (default 1 = combat-free)")
    ap.add_argument("--no-solo", action="store_true", help="skip the solo variant")
    ap.add_argument("--no-dm", action="store_true", help="skip the deathmatch variant")
    ...
    # which variants to run (both by default; --no-solo / --no-dm to skip one).
    # --bots / --solo-bots override the per-variant bot count baked in VARIANTS.
    overrides = {SOLO_VARIANT: args.solo_bots, DM_VARIANT: args.bots}
    variants = [(vn, nd, overrides.get(vn, nb)) for (vn, nd, nb) in VARIANTS
                if not (vn == SOLO_VARIANT and args.no_solo)
                and not (vn == DM_VARIANT and args.no_dm)]

    results = {}
    t0 = time.time()
    for m in maps:
        results[m] = {}
        for vname, _navdir, bots in variants:
            log(f"running {m} [{vname}] ({args.instances}x{args.seconds:.0f}s game, "
                f"{bots} bot{'s' if bots != 1 else ''}, seed {args.seed})...")
            met = run_map(engine, exe, dll_name, m, args, bots)
            if met:
                ic = met.get("item_completion")
                extra = (f"  frags {met['frags']}" if vname == DM_VARIANT else "")
                log(f"  {m} [{vname}]: {'COLLECT' if vname == SOLO_VARIANT else 'ITEM'} "
                    f"{fmt_pct(ic)}  pickups {met['pickups']}{extra}  nav {met['nav_nodes']}")
            results[m][vname] = met
    ...
    record = {
        ...
        "rig": {"instances": args.instances, "seconds": args.seconds, "bots": args.bots,
                ...
                "cvars": args.cvar or []},
        "solo_rig": {"instances": args.instances, "seconds": args.seconds,
                     "bots": args.solo_bots, "skill": args.skill, "seed": args.seed,
                     "cvars": args.cvar or []},
        "maps": results,   # variant-nested: {map: {"solo": <metrics>, "deathmatch": <metrics>}}
